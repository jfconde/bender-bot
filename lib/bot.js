...
module.exports = (function () {
    ...
    bot.route = (opts) => {
        let processRouteFn,
            getRequestUser = function (fromId) {
                return DB.user.find({
                    where: {
                        userId: fromId
                    }
        ...
        processRouteFn = function (bot) {
            ...
            return function (msg) {
                getRequestUser(msg.from.id).then(function (user) {
                    let nextPromise,
                    ...
                    nextPromise.then(function (user) {
                        ...
                        if (opts.permissions) {
                            if (msg.currentUser) {
                                ...
                                Log.error('user (%s) tried to acess protected resource (%s) but has not subscribed.', msg.from.id, opts.event);
